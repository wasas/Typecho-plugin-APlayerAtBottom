...
class APlayerAtBottom_Plugin implements Typecho_Plugin_Interface
{
  ...
  public static function deactivate(){
    unlink(__DIR__ ."/settings.json"); // 删除 settings.json
  ...
  public static function header(){
    ...
    if (file_exists(__DIR__."/settings.json") == false) {
      $data = ["last_update" => time(), "settings" => [], "data" => []];
      $data["settings"] = ["id" => $id, "lrc" => $lrc_out, "autoplay" => $autoplay_out, "theme" => $theme, "volume" => $volume, "order" => $order_out, "api" => $api];
